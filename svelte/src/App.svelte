<script lang="ts">
  /**
   * OSSリポジトリ ミニ検索 — Svelte 5 (runes) 版。
   *
   * Vue/Angular と同じく「画面更新を書かない」。$state に置いた状態が変わると
   * 参照箇所が追従する。Svelte の特徴はこれが**コンパイル時に解決される**こと —
   * 実行時のフレームワーク本体をほとんど持たない。その効果は転送量の実測に出る。
   */
